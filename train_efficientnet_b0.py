"""
Standalone Training Runner for EfficientNet-B0 on NVIDIA GeForce RTX 3050 (4 GB VRAM).
...
def run_efficientnet_training():
    logger.info("=" * 75)
    logger.info("  STARTING EFFICIENTNET-B0 REAL TRAINING RUN")
    ...
    for bs in candidate_batch_sizes:
        try:
            ...
            # Load DataLoaders (Held-out test set is strictly quarantined)
    ...
    # Load history to extract best epoch balanced accuracy and MCC
    ...
    if os.path.exists(history_path):
        with open(history_path, 'r', encoding='utf-8') as hf:
            ...
            best_idx = training_result["best_epoch"] - 1
            if 0 <= best_idx < len(history_data.get('val_balanced_acc', [])):
                best_balanced_acc = round(float(history_data['val_balanced_acc'][best_idx]), 4)
            if 0 <= best_idx < len(history_data.get('val_mcc', [])):
                best_mcc = round(float(history_data['val_mcc'][best_idx]), 4)
    ...
    summary_path = os.path.join(METRICS_DIR, "efficientnet_b0_training_summary.json")
    full_summary = {
    ...
    print("\n" + "=" * 70)
    print("  EFFICIENTNET-B0 REAL TRAINING RUN SUMMARY")
    print("=" * 70)
    ...
    print(f"6.  Balanced Accuracy               : {best_balanced_acc}")
    print(f"7.  Matthews Correlation (MCC)      : {best_mcc}")
    print(f"8.  Actual Batch Size               : {full_summary['actual_batch_size']}")
    print(f"9.  GPU Used                        : {full_summary['gpu_name']}")
    print(f"10. Peak VRAM                       : {full_summary['peak_vram_mb']} MiB / {total_vram_mb} MiB")
    print(f"11. Checkpoint Path                 : {full_summary['checkpoint_path']}")
    print(f"12. Warnings / Errors               : {warnings_or_errors if warnings_or_errors else 'None'}")
    print("=" * 70)
